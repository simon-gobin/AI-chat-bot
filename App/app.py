...
class RoleplayAssistant:
    ...
    def init_story(self):
        ...
        chat_history.append({"role": "system",
                             "content": f"The user has chosen {language}. Let's create a character and story together by answer following question: "})

        if self.story_state['User Character'] is None:
            chat_history.append(
                {"role": "system", "content": "First, ask the user if he can introduction himself for the story"})
        ...
        if self.story_state['Situation'] is None:
            chat_history.append(
                {"role": "system", "content": "Finally, ask what are the situation and what stroy he want?"})
            reply = self.model_output_init(chat_history)
            print(reply)
            user_reply = input("> ")
            user_reply = self.translation_to_eng(user_reply)
        ...
        while True:
            ...
            if "show me" in user_input.lower():
                ...
                image_prompt = [
                    {"role": "system", "content": "You are an image prompt generator."},
                    {"role": "user",
                     "content": f"the user ask you create a image prompt in english base on this last 5 messages:\n{self.story_state['chat'][-5:]} max 70 carateres"}
